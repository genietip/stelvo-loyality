...
/** Pull an earn code out of scanned QR data (raw code or URL last segment). */
...
export default function EarnScreen() {
  ...
  const [result, setResult] = useState<ClaimEarnReply | null>(null);
  const [permission, requestPermission] = useCameraPermissions();
  const scannedRef = useRef(false);

  const claim = useClaimEarnCode({
    mutation: {
      onSuccess: async (res) => {
        // Wallet and feed must reflect the new balance immediately.
        await Promise.all([
          queryClient.invalidateQueries({ queryKey: getGetWalletQueryKey() }),
          queryClient.invalidateQueries({
            queryKey: getGetWalletBusinessQueryKey(res.merchant_id),
          }),
  ...
  const submit = (value: string) => {
    ...
    if (!clean) {
      setError("Enter an earn code.");
      return;
    }
    setError(null);
    claim.mutate({ data: { code: clean } });
  };
      ...
      <View style={[styles.root, { backgroundColor: c.background }]}>
        <View style={styles.form}>
          ...
          <Text style={[styles.sub, { color: c.mutedForeground }]}>
            You now have {result.total_points} points at {result.business_name}.
          </Text>
          <Pressable
            onPress={() => router.back()}
            style={({ pressed }) => [
      ...
      {scanning && canScan ? (
        <View style={styles.scannerWrap}>
          ...
          <View style={styles.scanOverlay}>
            ...
            <Text style={styles.scanHint}>Point at the earn QR code at checkout</Text>
          </View>
          <Pressable
        ...
        <View style={styles.form}>
          ...
          <Text style={[styles.title, { color: c.foreground }]}>Earn points</Text>
          <Text style={[styles.sub, { color: c.mutedForeground }]}>
            Scan the QR code at checkout, or type in the earn code, to claim your
            points.
          </Text>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              styles.submit,
              { backgroundColor: c.primary, opacity: pressed || claim.isPending ? 0.8 : 1 },
            ]}
          >
            {claim.isPending ? (
              <ActivityIndicator color="#fff" />
            ) : (
              <Text style={styles.submitText}>Claim points</Text>
            )}
          </Pressable>
